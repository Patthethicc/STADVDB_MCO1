...
export default function Page() {
  ...
  const [apiPath] = useState<string>(() => {
    try {
      if (typeof window === "undefined") return '/api/sales-by-gender'
      const url = new URL(window.location.href)
      const q = url.searchParams.get('api')
      return q || '/api/sales-by-gender'
    } catch (e) {
      console.log(e);
      return '/api/sales-by-gender'
    }
  })

  const { setTitle } = useHeaderTitle();
  useEffect(() => {
    setTitle("Reports: Gender-wise Sales");
  }, [setTitle]);
  ...
  const routeButtons = [
    { label: "Male", api: "/api/sales-by-gender-male" },
    { label: "Female", api: "/api/sales-by-gender-female" },
    { label: "M + F", api: "/api/sales-by-gender" },
  ]
        ...
        <div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
          ...
          <div className="px-4 lg:px-6">
            <ChartInteractive
              ...
              description="Male and Female Sales Statistics"
              dayOlap={false}
